...
class SequenceGenerator:
    ...
    def generate_sequence(self, length: int) -> Tuple[List[str], List[str]]:
        ...
        first_obs = self._weighted_choice(self.hmm._emissions[start_state])
        ...
        for _ in range(1, length):
            ...
            current_state = self._weighted_choice(self.hmm._transitions[current_state])
            states.append(current_state)

            # Neue Beobachtung aus dem aktuellen Zustand erzeugen
            obs = self._weighted_choice(self.hmm._emissions[current_state])
